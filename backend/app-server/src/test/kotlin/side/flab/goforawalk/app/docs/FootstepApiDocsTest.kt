...
class FootstepApiDocsTest : DocsTestSupport() {
  ...
  fun `기록-발자취-캘린더`() {
    ...
    createFootstep(user, LocalDate.of(2025, 5, 5)).save(footstepRepository)
    createFootstep(user, LocalDate.of(2025, 5, 15)).save(footstepRepository)
    createFootstep(user, LocalDate.of(2025, 5, 25)).save(footstepRepository)
    ...
    mockMvc.perform(
      ...
      .andDo(
        docs.document(
          ...
          queryParameters(
            parameterWithName("startDate").description("조회 시작 날짜 (YYYY-MM-DD 형식)"),
            parameterWithName("endDate").description("조회 종료 날짜 (YYYY-MM-DD 형식)")
          ),
          responseFields(
            fieldWithPath("data.footsteps").description("발자취 리스트").type(ARRAY),
            fieldWithPath("data.footsteps[].userId").description("유저 ID").type(NUMBER),
            fieldWithPath("data.footsteps[].userNickname").description("유저 닉네임").type(STRING),
            fieldWithPath("data.footsteps[].footstepId").description("발자취 ID").type(NUMBER),
            fieldWithPath("data.footsteps[].date").description("발자취 날짜 (YYYY-MM-DD 형식)").type(STRING),
            fieldWithPath("data.footsteps[].imageUrl").description("이미지 URL").type(STRING),
            fieldWithPath("data.footsteps[].content").description("오늘의 한 마디").type(STRING).optional(),
            fieldWithPath("data.footsteps[].createdAt").description("생성 일시 (ISO-8601 형식)").type(STRING)
          )
        )
      )
  }
